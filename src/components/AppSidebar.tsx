...
export function AppSidebar() {
  ...
  return (
    <Sidebar className="bg-card/95 supports-[backdrop-filter]:bg-card/80 backdrop-blur border-r shadow-sm">
              ...
              {nav.map((item) => (
                <SidebarMenuItem key={item.title}>
                  <SidebarMenuButton asChild isActive={isActive(item.url)} tooltip={item.title}>
                    <NavLink to={item.url} end className={({ isActive }) => (isActive ? "rounded-md bg-primary/15 text-primary ring-1 ring-primary/30" : "rounded-md hover:bg-muted/60") }>
